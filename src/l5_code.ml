...
and compile_exp (e : L5_ast.exp) : (L4_ast.exp * L4_ast.func list) =
   ...
   | LambdaExp(p,vl,e) -> 
      ...
      let param = get_unique_ident l5_prefix in
      let (e2,fl) = compile_exp e in
      let (e3,_) = List.fold_right (fun v (ex,k) ->
         (L4_ast.LetExp(p,compile_var v,L4_ast.ArefExp(p,L4_ast.VarExp(p,L4_ast.Var(p,param)),
                                           L4_ast.IntExp(p,Int64.of_int k)),ex), k-1)
      ) vl (e2,(List.length vl)-1) in
      (L4_ast.LabelExp(p,name),
       (L4_ast.Function(p,name,[L4_ast.Var(p,param)],e3))::fl)
   ...
   | AppExp(p,e,el) -> 
      ...
      (L4_ast.AppExp(p,e2,[L4_ast.NewTupleExp(p,el2)]),fl1@fl2)
   (*| PrimExp(p,pr) -> *)

...
let rec compile_program (p : L3_ast.program) : L2_ast.program =
   ...
   | Program(p,e,fl) -> 
      ...
      let (_,fl2) = List.fold_left (fun (count,res) f ->
         (count+1,res@[compile_func f (get_unique_ident l3_prefix (*count*))])
      ) (1,[]) fl in (* start with the unique id "1", since we use "0" for
                      * the exp "e" *)
      let main = (L2_ast.Function(p,None,compile_exp e true (get_unique_ident l3_prefix (*0*)))) in
...
and compile_func (f : L3_ast.func) (prefix : string) : L2_ast.func = 
   ...
   | Function(p,name,vl,e) ->
      ...
      let il2 = compile_exp e false (get_unique_ident l3_prefix (*prefix*) (*1*)) in
...
and compile_exp (e : L3_ast.exp) (first : bool) (prefix : string) : L2_ast.instr list = 
   ...
   | LetExp(p,v,de,e) ->
      ...
      (compile_dexp de v2 false (get_unique_ident l3_prefix (*prefix 0*)))@
      (* compile the "e" *)
      (compile_exp e first (get_unique_ident l3_prefix (*prefix 1*)))
   | IfExp(p,sv,e1,e2) ->
      let tv = L2_ast.get_tval (compile_sval sv) in
      let il1 = (compile_exp e1 first (get_unique_ident l3_prefix (*prefix 0*))) in
      let il2 = (compile_exp e2 first (get_unique_ident l3_prefix (*prefix 1*))) in
      let tl = get_unique_ident l3_prefix (*prefix 2*) in
      let fl = get_unique_ident l3_prefix (*prefix 3*) in
      let jl = get_unique_ident l3_prefix (*prefix 4*) in
   ...
   | DExpExp(p,de) -> 
      ...
      (compile_dexp de (L2_ast.EaxReg(p)) tail (get_unique_ident l3_prefix (*prefix 0*)))@
...
and compile_dexp (de : L3_ast.dexp) (dest : L2_ast.var) (tail : bool) (prefix : string) : L2_ast.instr list = 
   ...
   | PlusDExp(p,sv1,sv2) ->
      ...
      let tmpd = L2_ast.Var(p,get_unique_ident l3_prefix (*prefix 0*)) in
   ...
   | TimesDExp(p,sv1,sv2) ->
      ...
      let tmp = L2_ast.Var(p,get_unique_ident l3_prefix (*prefix 0*)) in
      let sv1t = compile_sval sv1 in
   ...
   | ArefDExp(p,sv1,sv2) ->
      ...
      let sv2t = (compile_sval sv2) in
      let tmpd = L2_ast.Var(p,get_unique_ident l3_prefix (*prefix 0*)) in
      let tmp = L2_ast.Var(p,get_unique_ident l3_prefix (*prefix 1*)) in
      let tl1 = get_unique_ident l3_prefix (*prefix 1*) in
      let tl2 = get_unique_ident l3_prefix (*prefix 2*) in
      let fl = get_unique_ident l3_prefix (*prefix 3*) in
   ...
   | AsetDExp(p,sv1,sv2,sv3) ->
      ...
      let tmpd = L2_ast.Var(p,get_unique_ident l3_prefix (*prefix 0*)) in
      let tmp = L2_ast.Var(p,get_unique_ident l3_prefix (*prefix 1*)) in
      let tl1 = get_unique_ident l3_prefix (*prefix 2*) in
      let tl2 = get_unique_ident l3_prefix (*prefix 3*) in
      let fl = get_unique_ident l3_prefix (*prefix n4*) in
      [L2_ast.AssignInstr(p,tmpd,sv2t);
       L2_ast.SrlInstr(p,tmpd,L2_ast.IntShVal(p,1L));
       L2_ast.MemReadInstr(p,tmp,L2_ast.get_var sv1t,0L);
       L2_ast.LtJumpInstr(p,L2_ast.VarTVal(p,tmpd),L2_ast.VarTVal(p,tmp),tl1,fl);
...
and compile_var (v : L3_ast.var) : L2_ast.var = 
   ...
   let name = (try (make_ident_unique l3_prefix (List.find (fun x -> (x = raw)) resvd)) (* TODO - these unique var names look ugly *)
               with _ -> raw) in
   L2_ast.Var(p,name)
